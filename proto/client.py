...
if __name__ == '__main__':
  ...
  if (args.rotate == 'NONE'):
    ...
    print("Your rotation request needs to be one of: NONE, NINETY_DEG, ONE_EIGHTY_DEG, TWO_SEVENTY_DEG.")
  ...
  try:
    ...
    sys.exit(F"the input must be of either the PNG or JPEG format type. Your input was an {filename[1]} type.")
  ...
  if (result.width == -2):
    print("there was an error with the input type. unable to rotate or mean the file. please reconfirm your file type, or check to see if it is corrupted")
    sys.exit()
  elif (result.width == -1): 
    print("something went wrong while attempting to rotate or mean the input. there's probably a bug somewhere in the server.py file")
    sys.exit()
  ...
  im2.save(output, filename[len(filename) - 1])
  print("finished!")
