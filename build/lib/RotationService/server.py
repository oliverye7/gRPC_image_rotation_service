...
class NLImageServiceServicer(pb_grpc.NLImageServiceServicer):
    def checkInput(self, width, height, vals, color, context):
        """
        ...
        # error handling
        if color:
            if (width * height * 3 != len(vals)) or (len(vals) % 3 != 0):
                context.set_details(
                    "The input data does not represent a well formed image. Please reconfirm the dimensions or the file type."
                )
    ...
    def RotateImage(self, request, context):
        ...
        err = self.checkInput(width, height, vals, color, context)
        if err != 0:
            return err
    ...
    def MeanFilter(self, request, context):
        ...
        err = self.checkInput(width, height, data, color, context)
        if err != 0:
            return err
    ...
    def meanFilter(self, temp, temp2, height, width):
        ...
        for i in range(1, len(temp2) - 1):
            for j in range(1, len(temp2[0]) - 1):
                ...
                if (
                    ((i - 1 <= 0) and (j - 1 <= 0))
                    or ((i - 1 <= 0) and (j + 1 >= len(temp[0]) - 1))
                    or ((i + 1 >= len(temp) - 1) and (j - 1 <= 0))
                    or ((i + 1 >= len(temp) - 1) and (j + 1 >= len(temp[0]) - 1))
                ):
                    div = 4
                elif (
                    (i - 1 <= 0)
